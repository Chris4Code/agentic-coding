...
This is one of the better-corroborated models in the chapter once you look beyond Horthy's own account — though the corroboration is directional rather than a matching number. A GitHub Copilot field study of how developers spend their time (Hoffmann, Boysel, Nagle, Peng and Xu, *Generative AI and the Nature of Work*, reported via [Harvard Business Review](https://hbr.org/2025/03/how-is-your-team-spending-the-time-saved-by-gen-ai) and MIT Sloan) found developers with Copilot access shifted their time **+12.4% toward core coding and −24.9% away from project-management work**, with peer-collaboration time down sharply — consistent with the premise that coding itself is no longer the bottleneck, the surrounding decisions are. [Addy Osmani](https://addyosmani.com/blog/good-spec/) independently argues for the same shape of workflow, describing a rapid structured planning pass with an AI agent as "**waterfall in 15 minutes**." Microsoft's own [Spec-Driven Development](https://developer.microsoft.com/blog/spec-driven-development-ai-native-engineering/) framing and GitHub's Spec Kit tooling describe a parallel "Specify → Plan → Tasks → Implement" workflow, structurally similar to (though not derived from) Horthy's four gates. None of these independently name his specific 2–3x figure or four-gate structure — but the core thesis, that front-loading thinking compresses review into confirmation rather than discovery, converges from multiple unrelated directions.

Research Note: the quantitative support for this model is weaker than it first appears. The Copilot time-allocation study measures a proportional shift in how developers spend their hours, not a controlled comparison of planned versus unplanned agentic work, and the ~2–3x speedup is Horthy's own. An earlier draft of this section cited a "56% reduction in programming time" from a "joint MIT Sloan / Microsoft Research / GitHub study" as independent corroboration; that figure actually traces to a 2023 GitHub Copilot autocomplete experiment ([Peng et al.](https://arxiv.org/abs/2302.06590)) in which developers finished a greenfield JavaScript task 55.8% faster — a general coding-speed result with no spec-driven or planning intervention — so it is not evidence for this model and has been removed.
...
The result looks a lot like a business workflow, not a training run: work enters as a queue of tickets, gets routed through owned loops and delegated harnesses, and periodically needs a human — for a quality check, a compliance sign-off, or a security-critical approval — before it moves to the next stage. The rest of this section works through that in three directions: one concrete shape for the per-task loop itself ("Research, Plan, Implement" below), how the factory's topology should mature around it ("Make it run/right/fast"), what the workflow wrapped around it needs once real stakeholders enter the picture ("Agentic Business Workflows"), and the underlying reason none of this can just be solved with one bigger agent ("Why Not Just a Bigger Agent?").

### Research, Plan, Implement (RPI)

The **Ralph loop** — [Geoff Huntley](https://ghuntley.com/ralph/)'s deliberately unsophisticated pattern, a bare shell loop (`while :; do cat PROMPT.md | agent; done`) that runs one task per iteration against a fixed stack of instruction files and does a hard `git reset` when an iteration corrupts the tree — keeps each iteration's context small by brute force.

**Research, Plan, Implement (RPI)** is [Dex Horthy](https://github.com/humanlayer/12-factor-agents)'s more structured answer to the same constraint, aimed specifically at brownfield codebases, where an agent that starts editing before it understands the existing system tends to fix things in the wrong place. Horthy's essay [`ace-fca.md`](https://github.com/humanlayer/advanced-context-engineering-for-coding-agents/blob/main/ace-fca.md) ("Advanced Context Engineering for Coding Agents," from an August 2025 talk) presents it lowercase as a "research, plan, implement" workflow and as one instance of a broader family he calls **"frequent intentional compaction"** — deliberately structuring what enters the context window at every step. The "RPI" initialism comes from his later 2026 talks and the write-ups that followed, not the original essay.

The workflow splits a task into three phases, each run as its own agent session and each producing a durable markdown artifact that seeds the next:

1. **Research** — the agent documents how the relevant part of the codebase works *today*: which files are involved, how data flows, where functionality lives, with `file:line` references throughout. HumanLayer's own [`research_codebase.md`](https://github.com/humanlayer/humanlayer/blob/main/.claude/commands/research_codebase.md) prompt is emphatic that this phase only describes — "DO NOT suggest improvements or changes … ONLY describe what exists, where it exists, how it works" — and fans the work out to parallel read-only sub-agents whose stated purpose is "context control, not playing house": they absorb the search-and-read noise and return a distilled summary.
2. **Plan** — building on the research document, the agent writes an implementation plan: the exact steps, the files to change and how, and per-phase verification split into automated checks (runnable commands) and manual ones. The plan carries an explicit "what we are *not* doing" list, and the prompt forbids leaving any open question in a finished plan — "every decision must be made before finalizing."
3. **Implement** — the agent works through the plan phase by phase, checking items off *in the plan file itself*, running that phase's verification, then pausing for a human check before continuing. Only this phase needs a git worktree; research and planning produce nothing but markdown, committed on `main`.

```mermaid
flowchart LR
  T["Ticket / bug"] --> R["Research<br/>how the code works today"]
  R --> RD[["research.md"]]
  RD --> H1{{"human reads it,<br/>accepts or re-runs"}}
  H1 --> P["Plan<br/>exact steps + verification"]
  P --> PD[["plan.md"]]
  PD --> H2{{"human reviews<br/>the plan"}}
  H2 --> I["Implement<br/>phase by phase"]
  I -->|"compact status into plan.md,<br/>resume in a fresh session"| I
  I --> PR["Reviewed PR"]
```

The phase boundaries are deliberate context resets. Horthy's target is to keep context-window utilization in the **40–60% range** across the whole workflow; each phase starts a fresh session seeded only by the previous phase's artifact rather than the accumulated transcript, so the window never fills with the file-search output, tool logs, and stale reasoning that degrade a long-running session. He gives no token figures of his own, quoting instead Geoff Huntley's rule of thumb — roughly "170k of context window to work with … use as little of it as possible."

Separating research and planning from code generation is a leverage argument, not just context hygiene. In Horthy's framing, "a bad line of a plan could lead to hundreds of bad lines of code. And a bad line of research … could land you with thousands of bad lines of code" — so the review that pays off most is review of the research document and the plan, where a wrong assumption is still cheap to correct, not review of the eventual diff: "I can't read 2000 lines of golang daily. But I *can* read 200 lines of a well-written implementation plan." This is the same thesis as the [Leverage-Point Model](#leverage-point-model), one altitude down — that model's four gates are essentially RPI's Plan phase split into product, architecture, and program-design sub-gates, with Research folded into the architecture gate.

By early 2026 Horthy had revised the three phases into a longer pipeline he calls **QRSPI** — Questions, Research, Design (discussion), Structure (outline), Plan, Implement — presented in a talk pointedly titled "Everything We Got Wrong About Research-Plan-Implement." Three failure modes drove the revision. The planning prompt had grown past 85 instructions, beyond the roughly 150–200 a frontier model follows reliably before it starts silently dropping the steps buried deepest — usually the interactive "show me the options first" ones — so the monolith was split into sub-steps of under ~40 instructions each, the same "small, focused agents" discipline [the 12-factor-agents guide](https://github.com/humanlayer/12-factor-agents) argues for. It depended on "magic words": the workflow only behaved if the user pasted a specific trigger phrase, and by Horthy's own reasoning, "if a tool requires magic words for basic functionality, the tool itself is broken." And a detailed plan created a "plan-reading illusion" — it read convincingly whether or not the agent had actually understood the codebase, so "reading a plan is not the same as validating a plan." The added **Questions** and **Design discussion** steps force the open decisions into view before a plan is written. HumanLayer's shipped tooling now runs this revised sequence under the "RPI" name; community re-implementations rename it phonetically to *CRISPY*. The underlying split is unchanged: a reviewed *what*, then a reviewed *how*, then mechanical execution.

RPI is explicitly a brownfield technique. On greenfield work there is no existing system to research, and Horthy points instead to spec-first workflows (in the mould of Sean Grove's "specs are the new code") — the same distinction [spec-driven development](./quality.md#spec-driven-generation) draws. It is also applied selectively rather than to every task: HumanLayer reports roughly 40% of tasks skip straight to a one-shot attempt. It ships both as a linear checkpointed workflow and as Ralph-style loop commands (`ralph_research` / `ralph_plan` / `ralph_impl`) that pull the top ticket off a work board, run one phase, write its artifact, and move the ticket to an "in review" column.

Research Note: the mechanics above come directly from Horthy's essays and HumanLayer's published prompt files and are well documented. The QRSPI revision has no written essay behind it — it is sourced to the March 2026 talk — but the three failure modes and the phase list are reported consistently across several independent write-ups. The payoff figures are not independently audited. The "2–3x faster, safely" claim is self-reported, and the headline case study — a self-described "amateur Rust" contributor landing an approved bug-fix PR in a ~300k-line codebase within about an hour, then roughly 35k lines across two larger PRs in about seven hours (three on research and planning, four on implementation) — rests on real, checkable pull requests ([BoundaryML/baml #2259](https://github.com/BoundaryML/baml/pull/2259), #2357, #2330) but on Horthy's own account of the effort each took and a BAML maintainer's estimate that each "would have been 3–5 days of work for a senior engineer." Horthy also publishes failures, including a seven-hour dependency-removal attempt that went nowhere because the research phase did not follow the dependency tree deep enough.
