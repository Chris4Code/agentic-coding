...
Not every agentic software factory makes the same call about how much of the SDLC to hand to agents. [Dex Horthy](https://github.com/humanlayer/12-factor-agents), CEO of HumanLayer, frames the choice as a rough progression rather than a single fixed design: a pre-AI baseline (people decide → tracker → someone builds → PR review → ship → monitor → feedback loop) gives way, as agents take over more of the *building*, to three distinct models that differ mainly in where — and whether — a human still reads the code. The rest of this section covers each in turn.
