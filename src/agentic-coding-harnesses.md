...
Research Note: the "open-source" versus "commercial / managed" split above groups tools by model-vendor independence, not by the license of the client itself, so a reader who checks the repositories will find the boundary looser than the labels suggest. **OpenAI's Codex CLI** is itself open source (the `openai/codex` repository is Apache-2.0 licensed, originally TypeScript and later rewritten in Rust); it is nonetheless listed as vendor tooling because it only drives OpenAI's proprietary GPT-5-class models, authenticates through a paid ChatGPT plan or OpenAI API billing, and the wider Codex product it belongs to (the App Server / cloud mode, the IDE extension) is a proprietary managed service. **Google's Gemini CLI** (Apache-2.0) is the same situation — open client, but officially Gemini-only, authenticating via a Google account, a Gemini API key, or Vertex AI — and would sit in the same "commercial / managed" bucket for the same reason. **Alibaba's Qwen Code** (Apache-2.0) began as a fork of Gemini CLI v0.8.2 but has since diverged into independent development and is genuinely multi-protocol: it drives OpenAI-, Anthropic-, Gemini- and Qwen-compatible endpoints plus local models (Ollama / vLLM), switchable at runtime, and its optimization target — the Qwen models — ships as open weights, so it belongs with the model-agnostic open-source group rather than with Codex CLI and Gemini CLI. **Aider** (Apache-2.0) is correctly placed under open-source on the same criterion: it connects to almost any LLM including local models. Its documentation notes it performs best with specific commercial frontier models, and in practice it is most often paired with a commercial API — but that is a usage pattern, not a lock-in, and does not change the classification.
