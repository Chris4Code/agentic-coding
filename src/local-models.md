...
| NVIDIA B300 | 288 GB HBM3e | ~8 TB/s | Blackwell Ultra; the GB300 NVL72 generation, between B200 and Rubin |
| NVIDIA Rubin | ~288 GB HBM4 | ~19–22 TB/s per GPU (sources disagree — see below) | Successor to Blackwell; 50 PFLOPS NVFP4/GPU; ships to partners H2 2026 |
...
**NVLink** is NVIDIA's GPU-to-GPU fabric — 900 GB/s on Hopper, 1.8 TB/s per GPU on Blackwell (NVLink 5), 3.6 TB/s per GPU on Rubin (NVLink 6) — far above PCIe — and **NVSwitch** connects many GPUs all-to-all (a GB200/GB300 NVL72 rack is 72 Blackwell GPUs in one NVLink domain; a Vera Rubin NVL72 rack keeps the same 72-GPU domain size on Rubin). This is what makes tensor-parallel serving of a model too large for one GPU practical.

**NVIDIA Vera Rubin NVL72** is the rack-scale system built around the Rubin GPU, succeeding GB200/GB300 NVL72. Like GB200, it pairs a custom Arm CPU with GPUs on a shared module: one **Vera** CPU (88 custom "Olympus" Arm cores) plus two **Rubin** GPUs form a superchip, and 36 superchips fill the rack — 72 Rubin GPUs and 36 Vera CPUs total, the same package-level counting convention NVIDIA used for GB200 (each Rubin GPU package itself holds two compute dies, so the rack has roughly 144 dies, but NVIDIA's own marketing counts "GPUs" at the package level). NVIDIA's own spec sheet gives the rack 20.7 TB of HBM4 at 1,400 TB/s aggregate bandwidth, and — explicitly marked "preliminary" — up to 3,600 PFLOPS of NVFP4 inference compute; that HBM capacity is barely larger than the immediately-preceding GB300 NVL72's 20 TB (about a 3.5% increase), so most of NVIDIA's claimed generational gains (up to 10x lower inference token cost, 10x more tokens per megawatt vs. Blackwell) come from compute density, FP4 precision, and interconnect improvements rather than a larger memory pool. Partner systems are slated to ship in the second half of 2026, with AWS, Google, Microsoft, Meta, OpenAI, Anthropic, Oracle Cloud Infrastructure, CoreWeave, xAI, Cohere, Mistral AI, and Perplexity named as early customers. A separate SKU, **Rubin CPX**, pairs GDDR7 memory with dedicated attention/prefill hardware for long-context inference rather than HBM — NVIDIA's launch materials specifically name Cursor and Magic as design partners for large-codebase, long-context code generation, making it one of the few pieces of datacenter silicon marketed directly at agentic coding.

Research Note: NVIDIA's own primary sources disagree with each other on the Vera Rubin NVL72's aggregate NVLink bandwidth — its investor press release and developer blog state 260 TB/s, while its own product spec page states 216 TB/s for the same rack; both figures are reported here because neither could be confirmed as authoritative. Rack-level power draw is not officially published; secondary estimates range roughly 180–230 kW. Per-GPU HBM4 bandwidth is also inconsistent: secondary sources commonly cite ~22 TB/s, but dividing NVIDIA's own rack-total bandwidth by 72 GPUs gives ~19.4 TB/s.
